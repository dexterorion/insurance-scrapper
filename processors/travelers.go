...
// Travelers represents the Travelers structure
type Travelers struct{}

// Process processes html
func (an Travelers) Process(zip, state, city string) []models.Agent {
	executor := &outside.SeleniumExecutor{}
	executor.StartSelenium()

	agents := make([]models.Agent, 0)

	url := fmt.Sprintf("https://agent.travelers.com/search?q=%s&r=10", zip)
	...
	for {
		_, err := executor.WD.FindElement(selenium.ByCSSSelector, ".SpinnerModal-parent")
	...
	for _, htmlAgent := range htmlAgents {
		...
		name, err := htmlAgent.FindElement(selenium.ByCSSSelector, ".LocationName-brand")
		if err != nil {
			panic(err)
		}
		...
		phone, err := htmlAgent.FindElement(selenium.ByCSSSelector, ".Phone-link")
		if err != nil {
			panic(err)
		}
